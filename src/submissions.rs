...
use crate::{APIResponse, api_error};
...
pub async fn process_submissions(req: Request, ctx: RouteContext<()>) -> Result<Response> {
    ...
    let query = {
        ...
        let mut where_clauses = vec![];
        {
            ...
            where_clauses.push(format!("tasks.contest_name = ?{}", args.len()));
        }
        if let Some(task) = &query.task {
            ...
            where_clauses.push(format!("tasks.task_name = ?{}", args.len()));
        }
        ...
        if let Some(language) = &query.language {
            ...
            where_clauses.push(format!("languages.name = ?{}", args.len()));
        }
        ...
        stat.push_str("SELECT tasks.task_name AS task, users.name AS user, languages.name AS language, timestamp, status, code_size, score, execution_time FROM submissions, tasks, users, languages");
